...
def search_google(tld, country, language, result_per_page, queries, domain_to_find, save_html=False):
    results_list = []
...
if __name__ == "__main__":
    ...
    if st.button("Search"):
        if not queries or not domain_to_find:
            st.warning("Please provide both queries and the domain to track!")
        else:
            search_results_df = search_google(tld, country, language, result_per_page, queries, domain_to_find, save_html)
            st.success("Search completed!")

    if 'results_df' in st.session_state and not st.session_state.results_df.empty:
        user_choice = st.radio("Filter results by domain presence?", ("All Results", "Results where domain found"))
        filtered_df = st.session_state.results_df
        if user_choice == "Results where domain found":
            filtered_df = filtered_df[filtered_df["Domain Found"] == "Yes"]
